...
class DebugObserver
  def update(event, data)
    ...
    when :move_resolved
      # Only log first and last move step to reduce noise
    when :attack
...
monsters = 3.times.map { |i| monster_builder.as_bugbear.build.tap { |m| m.instance_variable_set(:@name, "Bugbear #{i+1}") } }
