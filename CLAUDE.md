...
| `/sdd:index` | Index a repository's ADRs, OpenSpec specs, and source code into qmd… |
| `/sdd:report-friction` | File a feedback issue against the SDD plugin (joestump/claude-plugin-sdd) when… |
| `/sdd:retro` | Comprehensive retrospective tool for analyzing failures and improving workflows |
| `/sdd:search` | Unified semantic exploration skill combining qmd hybrid retrieval with cgg… |
...
<!--
  ...
  CLAUDE.md, it replaces the `| Skill | Purpose |
|-------|---------|
| `/sdd:adr` | Create a new Architecture Decision Record (ADR) using MADR format |
| `/sdd:spec` | Create a specification with requirements, scenarios, and design rationale |
| `/sdd:list` | List all architecture decisions and specs with their status |
...
| `/sdd:report-friction` | File a feedback issue against the SDD plugin (joestump/claude-plugin-sdd) when… |
| `/sdd:retro` | Comprehensive retrospective tool for analyzing failures and improving workflows |
| `/sdd:search` | Unified semantic exploration skill combining qmd hybrid retrieval with cgg… |` marker with the
